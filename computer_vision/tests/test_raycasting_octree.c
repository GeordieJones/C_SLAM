...
// Macro for readable test assertion feedback
...
/**
 * 1. Lifecyle & Allocation Stability
 * Verifies memory initialization, node creation under custom pool allocation,
 * bounds assignment, and cleanup without memory leaks.
 */
void test_octree_lifecycle_and_allocation(void) {
    ...
    // Verify sub-allocations perform correctly within the initialized context
    ...
    #if USE_MEMORY_POOL
    // If pool is tracking metrics, verify allocation addresses are contiguous/clustered here
    #endif

    // Cleanup and release tracking handles
    Octree_free(root);
    Octree_free(child); 
}

/**
 * 2. Ray Geometry & Vector Math Consistency
 * Validates vector normalizations and edge-case ray vectors 
 * (e.g., parallel alignments, division by zero scenarios).
 */
void test_ray_math_and_directions(void) {
    ...
    // Assert normalized length is exactly 1.0f
    ...
    // Extreme Case: Identical origin and destination (Zero-length vectors)
    Point3D same_end = {0.0f, 0.0f, 0.0f};
    Point3D zero_dir = Ray_compute_direction(&origin, &same_end);
    // Ensure safety handling avoids NaN proliferation
    assert(!isnan(zero_dir.x) && !isnan(zero_dir.y) && !isnan(zero_dir.z));
}

/**
 * 3. Ray-AABB Intersection Math
 * Verifies accuracy of both Parametric / Kay-Kajiya methods 
 * and DDA Step boundaries across hits, misses, and grazing alignments.
 */
void test_ray_box_intersection_logic(void) {
    Point3D min_b = {1.0f, 1.0f, 1.0f};
    Point3D max_b = {3.0f, 3.0f, 3.0f};

    // Test Case A: Direct Center Hit
    ...
    // Test Case C: Ray originating inside the bounding box
...
/**
 * 4. Spatial Indexing Accuracy
 * Asserts coordinate-to-child index mappings match Octree octant rules.
 */
void test_octree_spatial_indexing(void) {
    ...
    // Test Point in Lower-Left-Front Octant (-X, -Y, -Z relative to center)
    Point3D pt_0 = {0.5f, 0.5f, 0.5f};
    int idx_0 = Octree_get_child_index(parent, &pt_0);
    assert(idx_0 >= 0 && idx_0 < 8);

    // Test Point in Top-Right-Back Octant (+X, +Y, +Z relative to center)
...
/**
 * 5. Ray Casting, Insertion, and Extraction Mechanics
 * Tests DDA updates and spatial traversal by writing a ray path 
 * into empty space and verifying voxel occupancy reports match.
 */
...
/**
 * 6. Temporal Stability and Invalidation State Machine
 * Validates frame-end state cooling (Active -> Static) transitions,
 * flag monitoring updates, and immediate bounding box localized invalidation.
 */
void test_stability_state_machine(void) {
    ...
    // Intersecting dynamic threat box triggers spatial awakening
    ...
    // Verify system sets region back to ACTIVE state upon collision contact
    assert(root->stability == NODE_ACTIVE);
...
/**
 * 7. Predictive Collision Guardrails
 * Verifies predictive bounding boxes flag intersections inside structural roots.
 */
