...
#include "../libaries/auto_labeler.h"
...
static CameraIntrinsics get_mock_intrinsics(void) {
    ...
    intrinsics.fx = 525.0f; // guess for focal length values
...
void test_circular_buffer_overflow(void) {
    ...
    for (uint32_t i = 0; i < flood_count; i++) {
        ...
        //  pass active references here, mimicking structural ingest pipeline steps.
